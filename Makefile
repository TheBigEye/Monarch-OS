...
# We maake the floppy image
OS.img: $(SOURCE_DIR)/boot/boot_main.bin #$(SOURCE_DIR)/boot/boot_zero.bin
	@echo -e "${GREEN}[-]${RESET} Generating system floppy image at '${BROWN}./$@${RESET}' ..."
	@cat $^ > $@
...
run: OS.img
	...
	@qemu-system-i386 -drive file=$<,format=raw $(QEMU_ARGS)
